...
const HabitPopUpContainer = ({ visibility, openPopUp, closePopUp }) => {
  ...
  const makeRequest = async (url, method, body) => {
    ...
    try {
      const response = await fetch(url, {
        ...
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json', // just send the header even if you have no body
        },
  ...
  const handleAiSubmit = async (event) => {
    ...
    const response = await makeRequest(askAiUrl, 'POST', { prompt: aiPrompt });
    closeAiModal();
    if (response) {
  ...
  return (
    <div className={`fixed inset-0 w-full h-screen flex flex-col justify-center items-center text-center bg-black bg-opacity-30`}>
      <div className='relative p-8 w-full max-w-2xl bg-gray-950 rounded-3xl'>
        <HabitNameDisplay handleInputChange={handleInputChange} habitNameInputValue={habitNameInputValue} />
        <br></br>
        <button className='bg-slate-900 hover:bg-slate-800 active:bg-slate-700 p-4 rounded-lg' onClick={openInnerPopUp}>
          Choose Genres
        </button>
        {innerVisibility && <ChooseGenrePopUpContainer genres={genres} closeInnerPopUp={closeInnerPopUp} handleCheckBoxChange={handleCheckBoxChange} checkedGenres={checkedGenres} />}
        ...
        <SliderDisplay targets={targets} handleSliderChange={handleSliderChange} energyValue={energyValue} danceabilityValue={danceabilityValue} valenceValue={valenceValue} />
        <form
          onSubmit={() => {
            handleSubmit();
            closePopUp();
        ...
        >
          <button onClick={closePopUp} className='bg-slate-900 hover:bg-slate-800 active:bg-slate-700 p-4 rounded-lg'>
            Cancel
          </button>
          <button type='submit' className='bg-slate-900 hover:bg-slate-800 active:bg-slate-700 p-4 rounded-lg'>
            GO
          </button>
          <button type='button' onClick={openAiModal} className='bg-slate-900 hover:bg-slate-800 active:bg-slate-700 p-4 rounded-lg'>
            Ask AI
          </button>
        </form>
        {aiModalVisible && (
          <div className='fixed inset-0 flex items-center justify-center bg-black bg-opacity-50'>
            <div className='bg-gray-900 p-6 w-1/2 h-1/4 rounded-3xl'>
              <form onSubmit={handleAiSubmit}>
                <textarea value={aiPrompt} onChange={handleAiPromptChange} placeholder='Enter your prompt here...' className='w-full h-48 p-2 border-2 border-gray-700 bg-gray-800 text-teal-500 rounded-xl' />
                <div className='flex justify-end mt-4'>
                  <button type='button' onClick={closeAiModal} className='bg-slate-800 hover:bg-slate-700 active:bg-slate-600 p-4 rounded-lg mx-4'>
                    Cancel
                  </button>
                  <button className='bg-slate-800 hover:bg-slate-700 active:bg-slate-600 p-4 rounded-lg mx-4' type='submit'>
                    Submit
                  </button>
                </div>
              </form>
